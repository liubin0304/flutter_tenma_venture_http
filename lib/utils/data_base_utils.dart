...
class DatabaseUtil {
  ...
  static Future<String> createDatabase() async {
    ...
    return join(databasesPath, 'unicorn_network_cache.db');
  }
  ...
  static Future<Database> openHttpDatabase(String path) async {
    Database database = await openDatabase(path,
      ...
      await db.execute(
          'CREATE TABLE If Not Exists UnicornNetworkCache (cacheKey TEXT PRIMARY KEY,  value TEXT)');
  ...
  static Future<List<Map<String, dynamic>>> queryHttp(
    ...
    return await database.rawQuery(
        'SELECT value FROM UnicornNetworkCache WHERE cacheKey = \'' +
            cacheKey +
            "\'");
  }
  ...
  static Future<int> insertHttp(
    ...
    return await database.transaction((txn) async {
      return await txn.rawInsert(
          'INSERT INTO UnicornNetworkCache(cacheKey, value) VALUES( \'' +
              cacheKey +
              '\', \'' +
              value +
              '\')');
  ...
  static Future<int> updateHttp(
    ...
    return await database
        .rawUpdate('UPDATE UnicornNetworkCache SET  value = \'' +
            value +
            '\' WHERE cacheKey = \'' +
            cacheKey +
            '\'')
  ...
  static Future<int> deleteHttp(Database database, String cacheKey) async {
    return await database.rawDelete(
        'DELETE FROM UnicornNetworkCache WHERE name = \'' + cacheKey + '\'');
  }
}
